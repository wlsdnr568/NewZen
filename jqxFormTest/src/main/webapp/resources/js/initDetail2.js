/**
 * 190104_mjw
 * 일반전표입력(acc01010) 하단 거래처 등록 스크립트(계좌번호와 사업용계좌가 나옴)
 * TODO jqxForm내 키 이벤트 처리
 */

//let isSearched; //사업자등록상태조회 변수   

define(["initDetail2", "jQuery", "jqxbuttons",
...
,function(initDetail2, jQuery, jqxButtons,
		jqxCheckBox, jqxInput, jqxListBox, 
		jqxScrollBar, jqxDropDownList, jqxRadioButton, 
		jqxPasswordInput, jqxNumberInput, jqxForm, 
	...
	//input요소에 포커스 됐을 때 값 전체 선택 + 배경색(파란색) 변화
	$("#pgGridCustForm2").on("focus", "input", function(e){
		e.preventDefault();
		var target = e.target;
		
		if(target.type == "text"){
			target.style.backgroundColor = "#e0e9f5"; //배경색(파란색) 변화
			target.select(); //값 전체 선택
		}
	})
	
	//input요소에 포커스 아웃 됐을 때 배경색(흰색) 변화
	$("#pgGridCustForm2").on("blur", "input", function(e){
		e.preventDefault();
		var target = e.target;
		
		if(target.type == "text"){
			target.style.backgroundColor = "white";//배경색(흰색) 변화
		}
	})
	...
	function initPgGridCustEditCompEvents(){
		...
		$("#pgGridCustForm2").on("setCodeHelper",function(event){
			
			let elemId = $("#pgGridCustForm2").jqxForm("getComponentNameById", event.args.id);
			
			if(elemId == "busnPersRegsNo" || elemId == "btnBusiness"){
				let busnPersRegsNoComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "busnPersRegsNo"); //사업자등록번호 component 불러오기
	...
	function searchAddr(){ 
		...
		let zipNo = $("#pgGridCustForm2").jqxForm("getComponentByName","zipNo");
		let addr = $("#pgGridCustForm2").jqxForm("getComponentByName","addr");
		let addrDtl = $("#pgGridCustForm2").jqxForm("getComponentByName","addrDtl");
	...
	function getCustTData(){
		$.ajax({
			url : fn_getContextPath() + "/codeHelper/getCustTData?CustCd=108",
			type : "get",
			dataType : "json",
			success : function(data){ 
				$.each(data, function(key, value){
					
					var custCdComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "custCd");
					custCdComp.val(value.custCd);//거래처코드 값으로 삽입
					
					var custNmComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "custNm");
					custNmComp.val(value.custNm);//거래처명 값으로 삽입
					
					var busnPersRegsNoComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "busnPersRegsNo");
					...
					var repstvNmComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "repstvNm");
					repstvNmComp.val(value.repstvNm);//대표자명 삽입
					
					var regsNoSecComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "regsNoSec");
					...
					var regsNoComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "regsNo");
					regsNoComp.val(value.regsNo);//계좌번호 삽입
					
					var addrComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "addr");
					addrComp.val(value.addr);//주소삽입
					
					var zipNoComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "zipNo");
					zipNoComp.val(value.zipNo);//우편번호 삽입
					
					var addrDtlComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "addrDtl");
					addrDtlComp.val(value.addrDtl);//우편번호 상세 삽입
					
					var bsnCdtComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "bsnCdt");
					bsnCdtComp.val(value.bsnCdt);//업태 삽입
					
					var itmComp = $("#pgGridCustForm2").jqxForm("getComponentByName", "itm");
					itmComp.val(value.itm);//종목 삽입
					
					var tel1Comp = $("#pgGridCustForm2").jqxForm("getComponentByName", "tel1");
					tel1Comp.val(value.tel1);
					
					var tel2Comp = $("#pgGridCustForm2").jqxForm("getComponentByName", "tel2");
					tel2Comp.val(value.tel2);
					
					var tel3Comp = $("#pgGridCustForm2").jqxForm("getComponentByName", "tel3");
		...
		.done(function(){
			formDataChange(); //페이지 로딩시 값 입력 후에 update이벤트 수해하도록 구성
		});
	}
	...
	function formDataChange(){
		$("#pgGridCustForm2").on("formDataChange",function(event){
			var args = event.args;
